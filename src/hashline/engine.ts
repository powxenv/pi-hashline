...
export function computeLineHash(idx: number, line: string): string {
  ...
  let seed = 0;
  if (!RE_SIGNIFICANT.test(trimmed)) {
    seed = idx;
  }
  return DICT[xxh32(trimmed, seed) & 0xff]!;
}
...
type Anchor = { line: number; hash: string; textHint?: string };
...
function formatMismatchError(
  ...
  for (const num of sorted) {
    ...
    out.push(
      retryLineSet.has(num) ? `>>> ${prefix}:${content ?? ""}` : `    ${prefix}:${content ?? ""}`,
    );
  }
  ...
  for (const edit of edits) {
    ...
    switch (op) {
      ...
      case "replace_text": {
        ...
        result.push({ op: "replace_text", oldText, newText });
        break;
      }
    }
  }
...
function cloneHashlineEdit(edit: HashlineEdit): HashlineEdit {
  switch (edit.op) {
    ...
    case "replace_text":
      return { op: "replace_text", oldText: edit.oldText, newText: edit.newText };
  }
}
...
function buildLineIndex(content: string): LineIndex {
  ...
  return { fileLines, lineStarts, hasTerminalNewline: content.endsWith("\n") };
}
...
function computeInsertionBoundary(
  ...
  const eofBoundary =
    lineIndex.hasTerminalNewline && fileLineCount > 0 ? fileLineCount - 1 : fileLineCount;
  ...
  switch (edit.op) {
    case "replace": {
      ...
      if (
        ...
        originalLines.every((line, li) => line === edit.lines[li])
      ) {
        noopEdits.push({
          editIndex: index,
    ...
    case "append": {
      ...
      const insertPoint = isSentinelAppend
        ? content.length
...
function computeLegacyEditLineRange(
  original: string,
  result: string,
